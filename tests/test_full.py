from brownie import Contract, LootChestArmor
import brownie
import pytest
...
def test_full(accounts, LootChestArmor):
    wep = LootChestArmor.deploy({"from": accounts[0]})
    ...
    print(wep.getChestArmor(1855))


def test_blacksmith(accounts, LootChestArmor, dai):
    wep = LootChestArmor.deploy({"from": accounts[0]})
